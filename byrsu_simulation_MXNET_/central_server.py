...
class Simulation:
    ...
    - dataset
    """
    def __init__(self, FCD_file, vehicle_dict: dict, rsu_list: list, central_server, training_set, val_train_data, val_test_data, train_data_byrsu, num_round):
        ...
        self.num_epoch = 0
        self.training_data = []
        self.training_data_byrsu = []
        self.training_label_byrsu = []
        self.train_data_byrsu = train_data_byrsu
    ...
    def new_epoch(self):
        ...
        if cfg['data_distribution'] == 'random':
            ...
            self.training_data_byrsu = []
            self.training_label_byrsu = []
            for X, y in self.train_data_byrsu.values():
                X_new = copy.deepcopy(X)
                y_new = copy.deepcopy(y)
                X_new, y_new = shuffle(np.array(X_new), np.array(y_new))
                self.training_data_byrsu.append(X_new.tolist())
                self.training_label_byrsu.append(y_new.tolist())
