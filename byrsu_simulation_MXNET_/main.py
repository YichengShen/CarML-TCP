            ...
            for vehicle in timestep.findall('vehicle'):
                ...
                if closest_rsu is not None:
                    ...
                    if cfg['data_distribution'] == 'random':
                        ...
                        if any(len(x) >= 100 for x in simulation.training_data_byrsu):
                            if len(simulation.training_data_byrsu[rsu_id]) >= 100:  
                                vehi.training_data_assigned = nd.array(np.array(simulation.training_data_byrsu[rsu_id][:100]))
                                del simulation.training_data_byrsu[rsu_id][:100]
                                vehi.training_label_assigned = nd.array(np.array(simulation.training_label_byrsu[rsu_id][:100]))
                                del simulation.training_label_byrsu[rsu_id][:100]
...
def main():
    ...
    simulation = Simulation(FCD_FILE, vehicle_dict, rsu_list, central_server, train_data, val_train_data, val_test_data, train_data_byrsu, num_round)
