    ...
    def __init__(self, ctx, rsu_list):
        ...
        if cfg['dataset'] == 'femnist':
            with self.net.name_scope():
                ...
                self.net.add(gluon.nn.Dense(62))
...
class Simulation:
    ...
    - dataset
    """
    def __init__(self, FCD_file, vehicle_dict: dict, rsu_list: list, central_server, training_set, val_train_data, val_test_data, partitioned_data, partitioned_labels, num_round): 
        ...
        self.partitioned_label = partitioned_labels
    ...
    def get_accu_loss(self):
        ...
        # accuracy on testing data
        outputs = self.central_server.net(nd.array(self.val_test_data[0]))
        self.epoch_accuracy.update(nd.array(self.val_test_data[1]), outputs)
        # cross entropy on training data
        outputs = self.central_server.net(nd.array(self.val_train_data[0]))
        self.epoch_loss.update(nd.array(self.val_train_data[1]), nd.softmax(outputs))
    ...
    def new_epoch(self):
        ...
        if cfg['data_distribution'] == 'random':
            ...
            self.training_data = []
            self.training_labels = []
            for rsu_id in range(cfg["simulation"]["num_rsu"]):
                X, y = shuffle(np.array(self.partitioned_data[rsu_id]), np.array(self.partitioned_label[rsu_id]))
                self.training_data.append(X.tolist())
                self.training_labels.append(y.tolist())
